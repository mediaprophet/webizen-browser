...
pub fn LinguisticsQapp() -> Element {
    ...
    let levels = ["Phonology", "Morphology", "Syntax", "Semantics", "Pragmatics", "Discourse Analysis", "Sociolinguistics", "Psycholinguistics"];
    let families = ["Indo-European", "Sino-Tibetan", "Afro-Asiatic", "Niger-Congo", "Austronesian", "Turkic", "Dravidian", "Japonic", "Koreanic", "Uralic", "Isolate"];

    rsx! {
        div {
            style: "padding: 20px; background: #1e1e2e; color: #cdd6f4; border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; gap: 16px; height: 100%; box-sizing: border-box;",
            h2 { style: "margin: 0; color: #94e2d5; border-bottom: 1px solid #313244; padding-bottom: 8px;", "Linguistics" }

            div {
                style: "display: grid; grid-template-columns: 1fr 1fr; gap: 12px;",
                div {
                    label { style: "font-size: 0.8rem; color: #a6adc8;", "Analysis Level" }
                    select {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "IPA Transcription" }
                input {
                    type: "text",
                    placeholder: "/ˈlɪŋ.ɡwɪ.stɪks/ — paste or type IPA symbols…",
                    value: "{ipa_input}",
                    oninput: move |e| ipa_input.set(e.value()),
                    style: "width: 100%; padding: 8px; background: #181825; border: 1px solid #45475a; color: #cdd6f4; border-radius: 4px; margin-top: 4px; font-family: serif; box-sizing: border-box;"
                }
            }

            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Morpheme Breakdown (use | to separate)" }
                input {
                    type: "text",
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Phrase Structure / Dependency Tree (bracket notation)" }
                textarea {
                    value: "{tree_notation}",
                    oninput: move |e| tree_notation.set(e.value()),
                    rows: "3",
                    placeholder: "[S [NP [Det The][N dog]] [VP [V chased][NP [Det the][N cat]]]]",
                    style: "width: 100%; padding: 8px; background: #181825; border: 1px solid #45475a; color: #cdd6f4; border-radius: 4px; margin-top: 4px; resize: vertical; font-family: monospace; box-sizing: border-box;"
                }
            }

            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Pragmatic / Discourse Context" }
                textarea {
            ...
            div {
                style: "background: #11111b; padding: 16px; border-radius: 8px; border-left: 4px solid #94e2d5;",
                h3 { style: "margin-top: 0; color: #94e2d5; font-size: 0.9rem;", "Analysis Output" }
                ...
                if !morpheme_input().is_empty() {
                    div { style: "margin-top: 8px; font-size: 0.85rem;",
                        ...
                        for morph in morpheme_input().split('|') {
                            span {
                                style: "background: #313244; padding: 2px 6px; border-radius: 3px; margin-right: 4px; font-size: 0.8rem;",
                                "{morph.trim()}"
                            }
                        }
                    }
                }
                div { style: "font-size: 0.75rem; color: #585b70; margin-top: 8px;", "QualiaDB → syntax_modeler | morphological parser | neuro-symbolic sieve" }
            }
        }
    }
}
