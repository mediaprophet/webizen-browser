...
pub fn WhiteStudiesQapp() -> Element {
    ...
    let foci = ["Whiteness as Invisible Norm", "White Privilege", "White Fragility", "White Nationalism", "Historical Construction of Whiteness", "Economic Whiteness"];
    let traditions = ["Critical Whiteness Studies", "CRT", "Labour Whiteness Studies", "Abolitionism"];
    let contexts = ["US", "UK", "Australia", "Europe", "South Africa", "Brazil", "Global"];
    let theorists = ["Roediger", "Frankenberg", "DiAngelo", "Morrison", "Ignatiev"];

    rsx! {
        div {
            style: "padding: 20px; background: #1e1e2e; color: #cdd6f4; border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; gap: 16px; height: 100%; box-sizing: border-box;",
            h2 { style: "margin: 0; color: #89dceb; border-bottom: 1px solid #313244; padding-bottom: 8px;", "White Studies" }
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Analytical Focus" }
                select {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Key Theorist" }
                select {
                    value: "{key_theorist}",
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Privilege Index: {privilege_index}" }
                input {
            ...
            div {
                ...
                span { style: "font-size: 0.8rem; color: #a6adc8;", "{analytical_focus} | {theoretical_tradition} | {context} | {key_theorist} | Privilege: {privilege_index}" }
                div { style: "font-size: 0.75rem; color: #585b70; margin-top: 6px;", "QualiaDB → white studies engine | norm sieve | privilege anchor" }
            }
        }
    }
}
