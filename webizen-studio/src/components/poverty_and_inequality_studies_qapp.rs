...
pub fn PovertyAndInequalityStudiesQapp() -> Element {
    ...
    let measures = [
        "Absolute Poverty Line", "Relative Poverty", "Multidimensional Poverty Index",
        "Capability Approach (Sen)", "Asset-Based",
    ];
    let metrics = [
        "Gini", "Palma Ratio", "Theil Index", "Atkinson Index", "Income Share Top 1%",
    ];
    let lenses = [
        "Structural", "Cultural", "Behavioural", "Feminist", "Postcolonial", "Rights-Based",
    ];
    let interventions = [
        "Cash Transfer", "Universal Basic Income", "Progressive Taxation",
        "Land Reform", "Education", "Healthcare",
    ];

    rsx! {
        div {
            style: "padding: 20px; background: #1e1e2e; color: #cdd6f4; border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; gap: 16px; height: 100%; box-sizing: border-box;",
            h2 { style: "margin: 0; color: #f9e2af; border-bottom: 1px solid #313244; padding-bottom: 8px;", "Poverty & Inequality Studies" }

            div {
                style: "display: grid; grid-template-columns: 1fr 1fr; gap: 12px;",
                div {
                    label { style: "font-size: 0.8rem; color: #a6adc8;", "Poverty Measure" }
                    select {
            ...
            div {
                style: "display: grid; grid-template-columns: 1fr 1fr; gap: 12px;",
                div {
                    label { style: "font-size: 0.8rem; color: #a6adc8;", "Gini Coefficient (0.0–1.0): {gini_coefficient:.2}" }
                    input {
                        r#type: "range",
                        min: "0",
                        max: "1",
                ...
                div {
                    label { style: "font-size: 0.8rem; color: #a6adc8;", "Poverty Headcount %: {poverty_headcount_pct:.1}" }
                    input {
                        r#type: "range",
                        min: "0",
                        max: "100",
            ...
            div {
                ...
                span { style: "font-size: 0.8rem; color: #a6adc8;", "{poverty_measure} | {inequality_metric}: {gini_coefficient:.2} | Headcount: {poverty_headcount_pct:.1}%" }
                div { style: "font-size: 0.75rem; color: #585b70; margin-top: 6px;", "QualiaDB → inequality engine | capability approach sieve | distribution graph" }
            }
        }
    }
}
