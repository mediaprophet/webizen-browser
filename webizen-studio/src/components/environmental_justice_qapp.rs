...
pub fn EnvironmentalJusticeQapp() -> Element {
    ...
    let justice_frameworks = ["Distributive", "Procedural", "Corrective", "Recognition", "Transformative"];
    let pollutant_burdens = ["Air Quality", "Water Contamination", "Toxic Waste", "Noise", "Heat Island"];
    let affected_communities = ["Low-Income", "Indigenous", "BIPOC", "Rural", "Urban", "Worker", "Global South"];

    let gap_color = if regulatory_gap() > 50 { "#f38ba8" } else { "#a6e3a1" };

    rsx! {
        div {
            style: "padding: 20px; background: #1e1e2e; color: #cdd6f4; border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; gap: 16px; height: 100%; box-sizing: border-box;",
            h2 { style: "margin: 0; color: #f38ba8; border-bottom: 1px solid #313244; padding-bottom: 8px;", "Environmental Justice" }
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Justice Framework" }
                select {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Pollutant Burden" }
                select {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Affected Community" }
                select {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Regulatory Gap: {regulatory_gap}" }
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Health Disparity: {health_disparity}" }
            ...
            div {
                ...
                span { style: "font-size: 0.8rem; color: #a6adc8;", "{justice_framework} | {pollutant_burden} | {affected_community} | gap: {regulatory_gap} | health: {health_disparity}" }
                div { style: "font-size: 0.75rem; color: #585b70; margin-top: 6px;", "QualiaDB → environmental justice engine | discourse sieve | anchor" }
            }
        }
    }
}
