...
pub fn ClinicalRiskScorer() -> Element {
    ...
    // Mock calculations
    let cvd_risk = if age() > 60 || smoker() { "High (24%)" } else { "Low (5%)" };
    let chad_score = (if age() >= 75 { 2 } else if age() >= 65 { 1 } else { 0 }) 
        + (if gender() == "Female" { 1 } else { 0 }) 
        + (if diabetes() { 1 } else { 0 })
        + 1; // Assuming hypertension

    let score_color = if cvd_risk.contains("High") { "#ff4d4d" } else { "#4dff88" };
