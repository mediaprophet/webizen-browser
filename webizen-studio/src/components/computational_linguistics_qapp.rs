...
pub fn ComputationalLinguisticsQapp() -> Element {
    ...
    let tasks = [
        "Parsing", "Named Entity Recognition", "Sentiment Analysis",
        "Machine Translation", "Coreference Resolution", "Speech Recognition", "Text Generation",
    ];
    let models = ["Rule-based", "Statistical", "Neural", "Transformer", "LLM"];
    let language_families = [
        "Indo-European", "Sino-Tibetan", "Afro-Asiatic",
        "Niger-Congo", "Dravidian", "Austronesian", "Other",
    ];
    ...
    rsx! {
        div {
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Task" }
                select {
                    value: "{task}",
            ...
            div {
                label { style: "font-size: 0.8rem; color: #a6adc8;", "Training Data Size (tokens)" }
                select {
            ...
            div {
                ...
                span { style: "font-size: 0.8rem; color: #a6adc8;", "{task} | {model} | {language_family} | acc {accuracy}% | {training_data_size} tokens" }
