use dioxus::prelude::*;

#[component]
pub fn MiddleEasternStudiesQapp() -> Element {
    let mut subregion = use_signal(|| "Levant".to_string());
    let mut period = use_signal(|| "Contemporary".to_string());
    let mut disciplinary_lens = use_signal(|| "Political Science".to_string());
    let mut language = use_signal(|| "Arabic".to_string());
    let mut research_notes = use_signal(|| String::new());

    rsx! {
        div {
            style: "padding: 20px; background: var(--qualia-surface); color: var(--qualia-text); border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; gap: 16px; height: 100%; box-sizing: border-box; overflow-y: auto;",
            h2 { style: "margin: 0; color: var(--qualia-accent); border-bottom: 1px solid var(--qualia-border); padding-bottom: 8px;", "Middle Eastern Studies QApp" }

            div { style: "display: flex; flex-direction: column; gap: 8px;",
                label { style: "font-size: 0.85rem; color: var(--qualia-text-muted);", "Sub-region" }
                select {
                    style: "background: var(--qualia-border); color: var(--qualia-text); border: 1px solid var(--qualia-border); border-radius: 6px; padding: 6px 8px; font-family: monospace; width: 100%; box-sizing: border-box;",
                    onchange: move |e| subregion.set(e.value()),
                    option { "Arabian Peninsula" }
                    option { selected: true, "Levant" }
                    option { "Iraq" }
                    option { "Iran" }
                    option { "Turkey" }
                    option { "Egypt" }
                    option { "Maghreb" }
                    option { "Gulf States" }
                    option { "Kurdish Regions" }
                }
            }

            div { style: "display: flex; flex-direction: column; gap: 8px;",
                label { style: "font-size: 0.85rem; color: var(--qualia-text-muted);", "Period" }
                select {
                    style: "background: var(--qualia-border); color: var(--qualia-text); border: 1px solid var(--qualia-border); border-radius: 6px; padding: 6px 8px; font-family: monospace; width: 100%; box-sizing: border-box;",
                    onchange: move |e| period.set(e.value()),
                    option { "Pre-Islamic" }
                    option { "Umayyad" }
                    option { "Abbasid" }
                    option { "Ottoman" }
                    option { "Colonial" }
                    option { "Post-Independence" }
                    option { selected: true, "Contemporary" }
                }
            }

            div { style: "display: flex; flex-direction: column; gap: 8px;",
                label { style: "font-size: 0.85rem; color: var(--qualia-text-muted);", "Disciplinary Lens" }
                select {
                    style: "background: var(--qualia-border); color: var(--qualia-text); border: 1px solid var(--qualia-border); border-radius: 6px; padding: 6px 8px; font-family: monospace; width: 100%; box-sizing: border-box;",
                    onchange: move |e| disciplinary_lens.set(e.value()),
                    option { selected: true, "Political Science" }
                    option { "History" }
                    option { "Literature" }
                    option { "Anthropology" }
                    option { "Islamic Studies" }
                    option { "Gender Studies" }
                    option { "Security Studies" }
                    option { "Energy Politics" }
                }
            }

            div { style: "display: flex; flex-direction: column; gap: 8px;",
                label { style: "font-size: 0.85rem; color: var(--qualia-text-muted);", "Language" }
                select {
                    style: "background: var(--qualia-border); color: var(--qualia-text); border: 1px solid var(--qualia-border); border-radius: 6px; padding: 6px 8px; font-family: monospace; width: 100%; box-sizing: border-box;",
                    onchange: move |e| language.set(e.value()),
                    option { selected: true, "Arabic" }
                    option { "Persian" }
                    option { "Turkish" }
                    option { "Hebrew" }
                    option { "Kurdish" }
                    option { "Amazigh" }
                }
            }

            div { style: "display: flex; flex-direction: column; gap: 8px;",
                label { style: "font-size: 0.85rem; color: var(--qualia-text-muted);", "Research Notes" }
                textarea {
                    style: "background: var(--qualia-border); color: var(--qualia-text); border: 1px solid var(--qualia-border); border-radius: 6px; padding: 6px 8px; font-family: monospace; width: 100%; box-sizing: border-box; min-height: 80px; resize: vertical;",
                    placeholder: "Research notes...",
                    oninput: move |e| research_notes.set(e.value()),
                }
            }

            div {
                style: "background: var(--qualia-bg); padding: 16px; border-radius: 8px; border-left: 4px solid var(--qualia-accent); flex: 1;",
                h3 { style: "margin-top: 0; color: var(--qualia-accent); font-size: 0.9rem;", "QualiaDB Output" }
                div { style: "font-size: 0.8rem; color: var(--qualia-text-muted); display: flex; flex-direction: column; gap: 4px;",
                    div { "Sub-region: {subregion()}" }
                    div { "Period: {period()}" }
                    div { "Lens: {disciplinary_lens()}" }
                    div { "Language: {language()}" }
                }
                div { style: "font-size: 0.75rem; color: var(--qualia-text-muted); margin-top: 8px;", "QualiaDB → knowledge graph | Allen Interval | geospatial" }
            }
        }
    }
}
